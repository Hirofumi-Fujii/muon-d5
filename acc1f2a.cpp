// acc1f2a.cpp
// g++ -Wall acc1f2a.cpp fit2dline.cpp hist2d.cpp mytimer.cpp ncpng.cpp -o acc1f2a
//
// 2016-05-16 acc1f2a.cpp started
...
//	double dist = (distx + disty) * 0.5;
	...
	// It is assumed that
	// 0 is the lowest position in the direction (for all even-odd, and X-Y combinations)
		...
		for (int xpx2 = 0; xpx2 < NUM_X_PIXELS; xpx2++)
		{
			...
			for (int ypx1 = 0; ypx1 < NUM_Y_PIXELS; ypx1++)
			{
				...
				for (int xpx1 = 0; xpx1 < NUM_X_PIXELS; xpx1++)
				{
					...
					// Unit1 hit position
					double x1eh = (zx2 - x1ez) * rdx + x2;
					double x1oh = (zx2 - x1oz) * rdx + x2;
					double y1eh = (zy2 - y1ez) * rdy + y2;
					double y1oh = (zy2 - y1oz) * rdy + y2;
					if ( ((x1eh - x1e0) < 0.0) ||
					     ((x1eh - x1e0) > X_UNIT_W) ||
					     ((x1oh - x1o0) < 0.0) ||
					     ((x1oh - x1o0) > X_UNIT_W) )
					...
					if ( ((y1eh - y1e0) < 0.0) ||
						...
						wt0 = wt1 = 0.0;
					// Unit2 hit position
					double x2eh = (zx2 - x2ez) * rdx + x2;
					double x2oh = (zx2 - x2oz) * rdx + x2;
					double y2eh = (zy2 - y2ez) * rdy + y2;
					double y2oh = (zy2 - y2oz) * rdy + y2;
					if ( ((x2eh - x2e0) < 0.0) ||
					     ((x2eh - x2e0) > X_UNIT_W) ||
					     ((x2oh - x2o0) < 0.0) ||
					     ((x2oh - x2o0) > X_UNIT_W) )
...
//					(phi,cos(theta)) using fitted positions
	...
	mytimer.stop ();
	// Show results.
	std::string ofname = std::string("acc1f2a.csv");
