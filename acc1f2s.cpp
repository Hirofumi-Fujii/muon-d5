// acc1f2s.cpp
// g++ -Wall acc1f2s.cpp fit2dline.cpp hist2d.cpp mytimer.cpp ncpng.cpp -o acc1f2s
//
// 2016-05-13 previous version is copied to acc1f2b.cpp
...
static const int NUM_X_PIXELS = 96;
static const int NUM_Y_PIXELS = 96;
...
int main (int argc, char* argv[])
{
	...
	double distx = x2pxz - x1pxz;
	double disty = y2pxz - y1pxz;

//	double dist = (distx + disty) * 0.5;
	double scalex = UNIT_ZDIFF / distx;
	double scaley = UNIT_ZDIFF / disty;

	// The calculation will be done by the half of the channel size
	// solid angle = (hfdsx1 / distx) * (hfdsy / disty) 
	// detector area = hfdsx2 * hfdsy2
	...
	{
		y2 = ((double)(ypx2) + 0.5) * PIXEL_YSIZE + y2px0;
		for (int xpx2 = 0; xpx2 < NUM_X_PIXELS; xpx2++)
		{
			x2 = ((double)(xpx2) + 0.5) * PIXEL_XSIZE + x2px0;
			for (int ypx1 = 0; ypx1 < NUM_Y_PIXELS; ypx1++)
			{
				y1 = ((double)(ypx1) + 0.5) * PIXEL_YSIZE + y1px0;
				for (int xpx1 = 0; xpx1 < NUM_X_PIXELS; xpx1++)
				{
					x1 = ((double)(xpx1) + 0.5) * PIXEL_XSIZE + x1px0;
					// calculate the track
					...
					// Unit1 hit position
					double x1eh = (x2pxz - x1ez) * rdx + x2;
					double x1oh = (x2pxz - x1oz) * rdx + x2;
					double y1eh = (y2pxz - y1ez) * rdy + y2;
					double y1oh = (y2pxz - y1oz) * rdy + y2;
					if ( ((x1eh - x1e0) < 0.0) ||
					     ((x1eh - x1e0) > X_UNIT_W) ||
					     ((x1oh - x1o0) < 0.0) ||
					     ((x1oh - x1o0) > X_UNIT_W) )
					...
					if ( ((y1eh - y1e0) < 0.0) ||
						...
						wt0 = wt1 = 0.0;
					// Unit2 hit position
					double x2eh = (x2pxz - x2ez) * rdx + x2;
					double x2oh = (x2pxz - x2oz) * rdx + x2;
					double y2eh = (y2pxz - y2ez) * rdy + y2;
					double y2oh = (y2pxz - y2oz) * rdy + y2;
					if ( ((x2eh - x2e0) < 0.0) ||
					     ((x2eh - x2e0) > X_UNIT_W) ||
					     ((x2oh - x2o0) < 0.0) ||
					     ((x2oh - x2o0) > X_UNIT_W) )
...
///// OUTPUT section /////
	...
	ofh
		<< "<!DOCTYPE html>\n"
		<< "<html>\n"
		<< "<head><title>acc1f2s results</title></head>\n"
		<< "<body>\n"
		<< "<hr>\n<h1>acc1f2s results</h1>\n<hr>\n"
		<< std::endl;
