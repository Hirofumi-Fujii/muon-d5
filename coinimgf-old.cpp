				...
				while (getline (ifs, recstr))
				{
					    ...
					    (recstr.substr(0,4) == "COIN"))
					{
						...
						if ((ss >> coinrec))
						{
							...
							// Front X
							if (coinrec.AB1cluster (uidx1, maxhits))
							{
								xdat[0] = (double)(coinrec.Apos(uidx1)) + xepos0[0];
								xdat[1] = (double)(coinrec.Bpos(uidx1)) + xopos0[0];
								xpos[0] = xunit_zpos[0];
								xpos[1] = xunit_zpos[1];
								nxdat = 2;
							}
							else if (coinrec.A1cluster (uidx1, maxhits))
							{					
								xdat[0] = (double)(coinrec.Apos(uidx1)) + xepos0[0];
								xpos[0] = xunit_zpos[0];
								nxdat = 1;
							}
							else if (coinrec.B1cluster (uidx1, maxhits))
							{
								xdat[0] = (double)(coinrec.Bpos(uidx1)) + xopos0[0];
								xpos[0] = xunit_zpos[1];
								nxdat = 1;
							}
							else
							...
							if (coinrec.AB1cluster (uidy1, maxhits))
							{
								ydat[0] = 470.0 - (double)(coinrec.Apos(uidy1)) + yepos0[0];
								ydat[1] = 470.0 - (double)(coinrec.Bpos(uidy1)) + yopos0[0];
								ypos[0] = yunit_zpos[0];
								ypos[1] = yunit_zpos[1];
								nydat = 2;
							}
							else if (coinrec.A1cluster (uidy1, maxhits))
							{					
								ydat[0] = 470.0 - (double)(coinrec.Apos(uidy1)) + yepos0[0];
								ypos[0] = yunit_zpos[0];
								nydat = 1;
							}
							else if (coinrec.B1cluster (uidy1, maxhits))
							{
								ydat[0] = 470.0 - (double)(coinrec.Bpos(uidy1)) + yopos0[0];
								ypos[0] = yunit_zpos[1];
								nydat = 1;
							}
							else
							...
							// Rear X
							if (coinrec.AB1cluster (uidx2, maxhits))
							{
								xdat[nxdat + 0] = (double)(coinrec.Apos(uidx2)) + xepos0[1];
								xdat[nxdat + 1] = (double)(coinrec.Bpos(uidx2)) + xopos0[1];
								xpos[nxdat + 0] = xunit_zpos[2];
								xpos[nxdat + 1] = xunit_zpos[3];
								nxdat = nxdat + 2;
							}
							else if (coinrec.A1cluster (uidx2, maxhits))
							{					
								xdat[nxdat + 0] = (double)(coinrec.Apos(uidx2)) + xepos0[1];
								xpos[nxdat + 0] = xunit_zpos[2];
								nxdat = nxdat + 1;
							}
							else if (coinrec.B1cluster (uidx2, maxhits))
							{
								xdat[nxdat + 0] = (double)(coinrec.Bpos(uidx2)) + xopos0[1];
								xpos[nxdat + 0] = xunit_zpos[3];
								nxdat = nxdat + 1;
							}
							else
								x2ok = false;
							...
							// Rear Y
							if (coinrec.AB1cluster (uidy2, maxhits))
							{
								ydat[nydat + 0] = (double)(coinrec.Apos(uidy2)) + yepos0[1];
								ydat[nydat + 1] = (double)(coinrec.Bpos(uidy2)) + yopos0[1];
								ypos[nydat + 0] = yunit_zpos[2];
								ypos[nydat + 1] = yunit_zpos[3];
								nydat = nydat + 2;
							}
							else if (coinrec.A1cluster (uidy2, maxhits))
							{					
								ydat[nydat + 0] = (double)(coinrec.Apos(uidy2)) + yepos0[1];
								ypos[nydat + 0] = yunit_zpos[2];
								nydat = nydat + 1;
							}
							else if (coinrec.B1cluster (uidy2, maxhits))
							{
								ydat[nydat + 0] = (double)(coinrec.Bpos(uidy2)) + yopos0[1];
								ypos[nydat + 0] = yunit_zpos[3];
								nydat = nydat + 1;
							}
							else
								y2ok = false;
