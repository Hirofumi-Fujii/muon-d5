...
//
	...
	static const int NUM_HIST2D = 24;
	mylibrary::Hist2D hist2d [NUM_HIST2D] =
	{
		Hist2D ("0:front X-Y ave", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("0:front X-Y fit", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("1:front X-Y ave", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("1:front X-Y fit", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("2:front X-Y ave", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("2:front X-Y fit", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("0:rear X-Y ave", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("0:rear X-Y fit", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("1:rear X-Y ave", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("1:rear X-Y fit", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("2:rear X-Y ave", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("2:rear X-Y fit", -2.5, 482.5, 97, -2.5, 482.5, 97),
		Hist2D ("0:dx-dy ave", -482.5, 482.5, 193, -482.5, 482.5, 193),
		Hist2D ("0:dx-dy fit", -482.5, 482.5, 193, -482.5, 482.5, 193),
		Hist2D ("1:dx-dy ave", -482.5, 482.5, 193, -482.5, 482.5, 193),
		Hist2D ("1:dx-dy fit", -482.5, 482.5, 193, -482.5, 482.5, 193),
		Hist2D ("2:dx-dy ave", -482.5, 482.5, 193, -482.5, 482.5, 193),
		Hist2D ("2:dx-dy fit", -482.5, 482.5, 193, -482.5, 482.5, 193),
		Hist2D ("0:xp-yp ave", xpmin, xpmax, 193, ypmin, ypmax, 193),
		Hist2D ("0:xp-yp fit", xpmin, xpmax, 193, ypmin, ypmax, 193),
		Hist2D ("1:xp-yp ave", xpmin, xpmax, 193, ypmin, ypmax, 193),
		Hist2D ("1:xp-yp fit", xpmin, xpmax, 193, ypmin, ypmax, 193),
		Hist2D ("1:xp-yp ave", xpmin, xpmax, 193, ypmin, ypmax, 193),
		Hist2D ("1:xp-yp fit", xpmin, xpmax, 193, ypmin, ypmax, 193),
	};
						...
						if ((ss >> coinrec))
						{
							...
							)
							{
								...
								if ((nxf > 0) && (nyf > 0) && (nxr > 0) && (nyr > 0))
								{
									...
									double xf_fit = xline.y (-(pxdist * 0.5));
									double yf_fit = yline.y (-(pydist * 0.5)) - uydiff;
									double xr_fit = xline.y (pxdist * 0.5);
									double yr_fit = yline.y (pydist * 0.5);
									double xp_fit = xline.y (obj_z) / 1000.0;
									double yp_fit = yline.y (obj_z) / 1000.0;

									double xf_ave = xsumf / (double)(nxf);
									double yf_ave = ysumf / (double)(nyf);
									double xr_ave = xsumr / (double)(nxr);
									double yr_ave = ysumr / (double)(nyr);
									double xp_ave = ((xr_ave - xf_ave - uxdiff) / pxdist * (obj_z - xlayer_zpos[3])
												 + xr_ave) / 1000.0;
									double yp_ave = ((yr_ave - yf_ave - uydiff) / pydist * (obj_z - ylayer_zpos[3])
												 + yr_ave) / 1000.0;

									hist2d[0].cumulate(xf_ave, yf_ave);
									hist2d[1].cumulate(xf_fit, yf_fit);
									hist2d[6].cumulate(xr_ave, yr_ave);
									hist2d[7].cumulate(xr_fit, yf_fit);

									if (allhcoin)
									{
										hist2d[2].cumulate(xf_ave, yf_ave);
										hist2d[3].cumulate(xf_fit, yf_fit);
										hist2d[8].cumulate(xr_ave, yr_ave);
										hist2d[9].cumulate(xr_fit, yr_fit);
									}
									if (all1cevent)
									{
										hist2d[4].cumulate(xf_ave, yf_ave);
										hist2d[5].cumulate(xf_fit, yf_fit);
										hist2d[10].cumulate(xr_ave, yr_ave);
										hist2d[11].cumulate(xr_fit, yr_fit);
									}
									double dx_ave = xf_ave - xr_ave;
									double dy_ave = yf_ave - yr_ave;
									double dx_fit = xf_fit - xr_fit;
									double dy_fit = yf_fit - yr_fit;
									hist2d[12].cumulate(dx_ave, dy_ave);
									hist2d[13].cumulate(dx_fit, dy_fit);
									if (allhcoin)
									{
										hist2d[14].cumulate(dx_ave, dy_ave);
										hist2d[15].cumulate(dx_fit, dy_fit);
									}
									if (all1cevent)
									{
										hist2d[16].cumulate(dx_ave, dy_ave);
										hist2d[17].cumulate(dx_fit, dy_fit);
									}
									hist2d[18].cumulate(xp_ave, yp_ave);
									hist2d[19].cumulate(xp_fit, yp_fit);
									if (allhcoin)
									{
										hist2d[20].cumulate(xp_ave, yp_ave);
										hist2d[21].cumulate(xp_fit, yp_fit);
									}
									if (all1cevent)
									{
										hist2d[22].cumulate(xp_ave, yp_ave);
										hist2d[23].cumulate(xp_fit, yp_fit);
									}
								}
							}
						}
