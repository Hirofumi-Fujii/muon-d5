...
//
// 2018-06-12 Projection option is added (VERSION = 103)
...
int main (int argc, char* argv[])
{
	...
	{
		tnxmin = (xmin + xdiff) / xdist;
		tnxmax = (xmax + xdiff) / xdist;
		tnxstep = (tnxmax - tnxmin) / double (nxbins);
		tnymin = (ymin + ydiff) / ydist;
		tnymax = (ymax + ydiff) / ydist;
		tnystep = (tnymax - tnymin) / double (nybins);
	}
	...
	for (int i = 0; i < NUMHIST2D; i++)
	{
		...
		else
		{
			...
			if (self_eocorr)
				...
				ofcsv << ",e/o corr,none";
			if (opt.m_projection)
				ofcsv << ",projection,true";
			else
				ofcsv << ",projection,false";
			ofcsv << std::endl;
			ofcsv
				<< "DAQTime(sec)"
				<< ',' << daqtime
